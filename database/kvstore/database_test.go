...
func TestDatabase_LoadTable(t *testing.T) {
	...
	err := db.LoadTable(testCSV, "test", "d6") //Rollable table because of re
	if err != nil {
		t.Errorf("error loading table was not expected, but err was encountered %s\n", err)
	}
	err = db.LoadTable(testCSV, "test2", "") //Standard table because no re
	if err != nil {
...
func TestDatabase_GetTable(t *testing.T) {
	...
	err := db.LoadTable(testCSV, "test", "d6")
	if err != nil {
	...
	for i, row := range results {
		...
		case 0:
			if (row[0] != "D6") || (row[1] != "Result") || (row[2] != "Description") {
				t.Errorf("expected to contain [D6][Result][Description], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 1:
			if (row[0] != "1") || (row[1] != "Fight {{1d1}} rats") || (row[2] != "The party runs across some dirty rats.") {
				t.Errorf("expected to contain [1][Fight {{1d1}} rats][The party runs across some dirty rats.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 2:
			if (row[0] != "2") || (row[1] != "No encounter") || (row[2] != "Nothing to see here.") {
				t.Errorf("expected to contain [2][No encounter][Nothing to see here.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 3:
			if (row[0] != "3") || (row[1] != "A wolf can be heard nearby") || (row[2] != "If the party is careful they may avoid the wolf.") {
				t.Errorf("expected to contain [3][A wolf can be heard nearby][If the party is careful they may avoid the wolf.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 4:
			if (row[0] != "4") || (row[1] != "{{1d1+1}} bats attack") || (row[2] != "Angry bats swarm and attack the party.") {
				t.Errorf("expected to contain [4][{{1d1+1}} bats attack][Angry bats swarm and attack the party.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 5:
			if (row[0] != "5") || (row[1] != "I can see you, can you see me?") || (row[2] != "A whisper can be heard in the trees.") {
				t.Errorf("expected to contain [5][I can see you, can you see me?][A whisper can be heard in the trees.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case 6:
			if (row[0] != "6") || (row[1] != "A pile of bones covers {{1d1}}GP") || (row[2] != "You found some loot.") {
				t.Errorf("expected to contain [6][A pile of bones covers {{1d1}}GP][You found some loot.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		}
	}
...
func TestDatabase_TableExpression(t *testing.T) {
	...
	if (results[0][0] != "D6") || (results[0][1] != "Result") || (results[0][2] != "Description") {
		t.Errorf("expected to contain [D6][Result][Description], actual values [%s][%s][%s]", results[0][0], results[0][1], results[0][2])
	}
	...
	for i, row := range results {
		...
		case "1":
			if (row[0] != "1") || (row[1] != "Fight 1 rats") || (row[2] != "The party runs across some dirty rats.") {
				t.Errorf("expected to contain [1][Fight 1 rats][The party runs across some dirty rats.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case "2":
			if (row[0] != "2") || (row[1] != "No encounter") || (row[2] != "Nothing to see here.") {
				t.Errorf("expected to contain [2][No encounter][Nothing to see here.], actual values [%s][%s][%s]", row[0], row[1], row[2])
		...
		case "4":
			if (row[0] != "4") || (row[1] != "2 bats attack") || (row[2] != "Angry bats swarm and attack the party.") {
				t.Errorf("expected to contain [4][2 bats attack][Angry bats swarm and attack the party.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		case "5":
			if (row[0] != "5") || (row[1] != "I can see you, can you see me?") || (row[2] != "A whisper can be heard in the trees.") {
				t.Errorf("expected to contain [5][I can see you, can you see me?][A whisper can be heard in the trees.], actual values [%s][%s][%s]", row[0], row[1], row[2])
		...
		case "6":
			if (row[0] != "6") || (row[1] != "A pile of bones covers 1GP") || (row[2] != "You found some loot.") {
				t.Errorf("expected to contain [6][A pile of bones covers 1GP][You found some loot.], actual values [%s][%s][%s]", row[0], row[1], row[2])
			}
		}
	}
	...
	if (results[1][0] != "3") || (results[1][1] != "A wolf can be heard nearby") || (results[1][2] != "If the party is careful they may avoid the wolf.") {
		t.Errorf("expected to contain [3][A wolf can be heard nearby][If the party is careful they may avoid the wolf.], actual values [%s][%s][%s]", results[1][0], results[1][1], results[1][2])
	}
...
func TestDatabase_RandomRow(t *testing.T) {
	...
	case 1:
		if (results[0] != "1") || (results[1] != "Fight 1 rats") || (results[2] != "The party runs across some dirty rats.") {
			t.Errorf("expected to contain [1][Fight 1 rats][The party runs across some dirty rats.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	case 2:
		if (results[0] != "2") || (results[1] != "No encounter") || (results[2] != "Nothing to see here.") {
			t.Errorf("expected to contain [2][No encounter][Nothing to see here.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	case 3:
		if (results[0] != "3") || (results[1] != "A wolf can be heard nearby") || (results[2] != "If the party is careful they may avoid the wolf.") {
			t.Errorf("expected to contain [3][A wolf can be heard nearby][If the party is careful they may avoid the wolf.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	case 4:
		if (results[0] != "4") || (results[1] != "2 bats attack") || (results[2] != "Angry bats swarm and attack the party.") {
			t.Errorf("expected to contain [4][2 bats attack][Angry bats swarm and attack the party.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	case 5:
		if (results[0] != "5") || (results[1] != "I can see you, can you see me?") || (results[2] != "A whisper can be heard in the trees.") {
			t.Errorf("expected to contain [5][I can see you, can you see me?][A whisper can be heard in the trees.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	case 6:
		if (results[0] != "6") || (results[1] != "A pile of bones covers 1GP") || (results[2] != "You found some loot.") {
			t.Errorf("expected to contain [6][A pile of bones covers 1GP][You found some loot.], actual values [%s][%s][%s]", results[0], results[1], results[2])
		}
	}
...
func TestDatabase_RandomRow_withRanges(t *testing.T) {
	...
	err := db.LoadTable(rangedCSV, "test", "d6")
	if err != nil {
		t.Errorf("error while loading table was not expected, but err was encountered %s\n", err)
	}
	...
	case 1:
		if (results[0] != "1-2") || (results[1] != "You rolled a 1 or 2") {
			t.Errorf("expected to contain [1-2][You rolled a 1 or 2], actual values [%s][%s]", results[0], results[1])
		}
	case 2:
		if (results[0] != "1-2") || (results[1] != "You rolled a 1 or 2") {
			t.Errorf("expected to contain [1-2][You rolled a 1 or 2], actual values [%s][%s]", results[0], results[1])
	...
	case 3:
		if (results[0] != "3-4") || (results[1] != "You rolled a 3 or 4") {
			t.Errorf("expected to contain [3-4][You rolled a 3 or 4], actual values [%s][%s]", results[0], results[1])
		}
	case 4:
		if (results[0] != "3-4") || (results[1] != "You rolled a 3 or 4") {
			t.Errorf("expected to contain [3-4][You rolled a 3 or 4], actual values [%s][%s]", results[0], results[1])
	...
	case 5:
		if (results[0] != "5-6") || (results[1] != "You rolled a 5 or 6") {
			t.Errorf("expected to contain [5-6][You rolled a 5 or 6], actual values [%s][%s]", results[0], results[1])
		}
	case 6:
		if (results[0] != "5-6") || (results[1] != "You rolled a 5 or 6") {
			t.Errorf("expected to contain [5-6][You rolled a 5 or 6], actual values [%s][%s]", results[0], results[1])
...
func TestDatabase_GetHeader(t *testing.T) {
	...
	if (results[0] != "D6") || (results[1] != "Result") || (results[2] != "Description") {
		t.Errorf("expected to contain [D6][Result][Description], actual values [%s][%s][%s]", results[0], results[1], results[2])
	}
...
func TestDatabase_ListTables(t *testing.T) {
	...
	err = db.LoadTable(testCSV, "test2", "")
	if err != nil {
		t.Errorf("error was not expected, but err was encountered %s\n", err)
	}
...
func TestDatabase_LoadTable_replaceStandard(t *testing.T) {
	...
	err = db.LoadTable(rangedCSV, "test", "")
	if err != nil {
		t.Errorf("error while loading table was not expected, but err was encountered %s\n", err)
	}
...
func TestErrors(t *testing.T) {
	...
	err = db.LoadTable(testCSV, "", "") //no re so Standard
	if err == nil {
		t.Errorf("expected an err, but none occured\n")
	}

	err = db.LoadTable(testCSV, "", "2d8") //re so Rollable
	if err == nil {
		t.Errorf("expected an err, but none occured\n")
	}

	err = db.LoadTable(badCSV, "bad", "d3")
	if err == nil {
		t.Errorf("expected an err, but none occured\n")
	} else {
	...
	err = db.LoadTable(testCSV, "testnoroll", "")
	if err != nil {
		t.Errorf("error while loading table was not expected, but err was encountered %s\n", err)
	}
	...
	err = db.LoadTable(testCSV, "testgood", "d6")
	if err != nil {
		t.Errorf("error while loading table was not expected, but err was encountered %s\n", err)
	}
